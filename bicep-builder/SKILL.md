...
Create or restructure Bicep for Azure workloads aligned to the Microsoft Well-Architected Framework, using a strict file decomposition model. Supports one deployable workload root or several sibling roots (e.g. `workloads/lims-data`, `workloads/lims-avd`) sharing resource-family modules in `workloads/modules/`.
...
- Bicep organised for maintenance, reviewability, and safe change isolation.
...
**Folder presence:** `main.bicep`, `params/`, `README.md`, `ADRs/` always exist in a deployable root. `config/` only when external JSON config is needed — never an empty folder. `builds/` holds generated ARM parameter artifacts only; never hand-edit them.
...
1. Ask all clarifying questions in one batch (intent, constraints, split vs single root).
2. Identify resource families; decide root split; record split rationale for the ADR.
3. Check/create `.gitignore`.
4. Propose the file tree; get confirmation before writing contents if the ask is large or ambiguous.
...
Run `scripts/validate-bicep.sh [root-path]` once, rather than issuing individual `bicep build` / `az bicep build-params` commands per file. It builds every shared module, every root's `main.bicep`, and every `.bicepparam` file into its matching `builds/` artifact, and fails loudly with a nonzero exit code if anything breaks. Do not deliver code until it exits 0. If it fails: fix syntax/parameter/type issues and re-run — don't hand-narrate each individual build command's output back to the user.
